...
- [ ] Реальное дерево улучшений (граф, раскрытие, покупка)
- [ ] Реальный баланс цен/прогрессии дерева

### 3) Уровень: управление и базовые системы (MVP → полноценная игра)
- [ ] Управление кораблём: WASD + инерция (ускорение/демпфирование)
- [ ] Поворот корабля к курсору
- [ ] Стрельба ЛКМ (пули, скорострельность, урон)
- [ ] Астероиды: спавн, хаотичное движение, HP
- [ ] Разрушение астероидов: эффекты, дроп минералов + шанс скрапа
- [ ] Сбор лута (pickup + притяжение/коллизии)
- [ ] Топливо: расход от движения, завершение рана при 0
- [ ] Урон от столкновений: на старте смертельно (конфигом)
- [ ] Завершение рана: возврат в меню с переносом добычи в прогресс

### 4) Враги и бой (после MVP астероидов)
- [ ] Спавн врагов по прогрессу (kill-count/time)
- [ ] AI врагов (преследование/держать дистанцию/паттерны)
- [ ] Пули врагов, попадания, урон
- [ ] Дроп с врагов (больше минералов/скрапа)

### 5) Босс и прохождение уровня
- [ ] Директор уровня: появление босса после условия
- [ ] Босс: несколько атак/телеграфы/фазы
- [ ] Условие победы: убийство босса
- [ ] Unlock следующего уровня после победы

### 6) Мета-прогрессия (полноценная)
- [ ] Модель апгрейдов (дерево) + требования открытия
- [ ] Применение апгрейдов к статам корабля (derive stats)
- [ ] Экономика: цены/дроп/темп апгрейдов (баланс)
...
- [ ] Save schema (zod) + версия
- [ ] `loadSave()/saveGame()` (localforage)
- [ ] Миграции save-версий
- [ ] Автосохранение при важных событиях (покупка апгрейда/конец рана)